...
export default function C133Form() {
  ...
  function RadioOption({ name, value, current, label, desc }) {
    ...
    return (
      <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {PRESTACIONES.map(p => (
            <RadioOption key={p.code} name="prestacion" value={p.code} current={data.prestacion} label={language === 'en' ? p.en : p.es} />
          ))}
        </div>
      </>
    );
                ...
                <tr key={idx}>
                  ...
                  <td style={{ padding: '0.3rem' }}>
                    <select className="input-field" style={{ ...inputStyle, width: '120px' }} value={row.mes} onChange={e => updateBase(idx, 'mes', e.target.value)}>
                      <option value="">--</option>
                      {(language === 'en' ? t('common.months') : t('common.monthsES')).map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
                    </select>
                  </td>
