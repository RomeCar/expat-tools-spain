...
export default function Modelo149Form() {
  ...
  function RadioOption({ name, value, current, label, desc, icon: Icon }) {
    ...
    return (
      <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', padding: '0.75rem 1rem', background: selected ? 'rgba(37, 99, 235, 0.08)' : 'var(--bg-tertiary)', border: `1.5px solid ${selected ? 'var(--accent-primary)' : 'var(--border-color)'}`, borderRadius: '0.5rem', cursor: 'pointer', transition: 'all 0.15s' }}>
        <input type="radio" name={name} checked={selected} onChange={() => update(name, value)} style={{ marginTop: '0.2rem', accentColor: 'var(--accent-primary)' }} />
        <div style={{ flex: 1 }}>
    ...
    return (
      <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <RadioOption name="taxpayerType" value="primary" current={data.taxpayerType} label={f.taxpayerType.primary} desc={f.taxpayerType.primaryDesc} />
          <RadioOption name="taxpayerType" value="associated" current={data.taxpayerType} label={f.taxpayerType.associated} desc={f.taxpayerType.associatedDesc} />
        </div>
  ...
  function Step3() {
    return (
      <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <RadioOption name="purpose" value="option" current={data.purpose} label={f.purpose.option} desc={f.purpose.optionDesc} />
          <RadioOption name="purpose" value="renunciation" current={data.purpose} label={f.purpose.renunciation} desc={f.purpose.renunciationDesc} />
          <RadioOption name="purpose" value="exclusion" current={data.purpose} label={f.purpose.exclusion} desc={f.purpose.exclusionDesc} />
          <RadioOption name="purpose" value="endDisplacement" current={data.purpose} label={f.purpose.endDisplacement} desc={f.purpose.endDisplacementDesc} />
        </div>
        {data.purpose === 'option' && (
          <div style={{ marginTop: '1rem', padding: '1rem', background: 'var(--bg-tertiary)', borderRadius: '0.5rem' }}>
  ...
  function Step4() {
    if (data.taxpayerType === 'associated') {
      return (
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <RadioOption name="associatedType" value="spouse" current={data.associatedType} label={f.dates.spouse} />
            <RadioOption name="associatedType" value="parent" current={data.associatedType} label={f.dates.parent} />
            <RadioOption name="associatedType" value="child" current={data.associatedType} label={f.dates.child} />
          </div>
    ...
    return (
      <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <RadioOption name="category" value="employment" current={data.category} label={f.situation.employment} desc={f.situation.employmentDesc} icon={Briefcase} />
          <RadioOption name="category" value="director" current={data.category} label={f.situation.director} desc={f.situation.directorDesc} icon={Building2} />
          <RadioOption name="category" value="entrepreneur" current={data.category} label={f.situation.entrepreneur} desc={f.situation.entrepreneurDesc} icon={Rocket} />
          <RadioOption name="category" value="professional" current={data.category} label={f.situation.professional} desc={f.situation.professionalDesc} icon={GraduationCap} />
          <RadioOption name="category" value="research" current={data.category} label={f.situation.research} desc={f.situation.researchDesc} icon={FlaskConical} />
        </div>
        ...
        <div style={{ marginTop: '1rem', padding: '1rem', background: 'var(--bg-tertiary)', borderRadius: '0.5rem' }}>
          {data.category === 'employment' && (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }}>
                <RadioOption name="employmentType" value="newJob" current={data.employmentType} label={f.situation.newJob} />
                <RadioOption name="employmentType" value="transfer" current={data.employmentType} label={f.situation.transfer} />
                <RadioOption name="employmentType" value="remote" current={data.employmentType} label={f.situation.remote} />
              </div>
          ...
          {data.category === 'director' && (
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }}>
                <RadioOption name="directorType" value="standard" current={data.directorType} label={f.situation.dirStandard} />
                <RadioOption name="directorType" value="patrimonial" current={data.directorType} label={f.situation.dirPatrimonial} />
              </div>
  ...
  // Determine which steps to show (skip step 4 for associated if purpose isn't 'option')
